    ...
    <el-card>
      <el-form :inline="true" :model="navObj" class="demo-form-inline">
        <el-form-item label="企业编号">
          <el-input class="short" v-model="navObj.eid"></el-input>
        </el-form-item>
      ...
      <el-table :data="enterpriseList" style="width: 100%">
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            ...
            <el-link type="primary" @click="deleteenterprise(scope.row)">删除</el-link>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
...
//导入新增面版
import addenterprise from "./components/adddialog";
//导入编辑面板
import editenterprise from "./components/editdialog";
export default {
  data() {
    return {
      // 页容量的数据
  ...
  methods: {
    ...
    edit(row) {
      ...
      //直接渲染会出现同步修改的bug
      // this.$refs.editenterprise.form = row;
      // // 解决同步修改的 bug
    ...
    deleteenterprise(row) {
      this.$confirm("是否确定删除?", "提示", {
        ...
        .then(res => {
          console.log(res);
          console.log(row);
          apiDeleteenterprise(row).then(res => {
